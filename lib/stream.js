...
TwitterStream.prototype.connect = function(reconnect) {
  ...
  function onResponse(stream) {
    ...
    function onData() {
      ...
      timeout = setTimeout(onStreamTimeout, constants.TIMEOUT);
    }

    var timeout = setTimeout(onStreamTimeout, constants.TIMEOUT);
    ...
    function cleanup() {
      ...
      stream.destroy();
      jstream.removeListener('error', onResponseError);
      jstream.close();
      clearTimeout(timeout);
      delete self._stream;
