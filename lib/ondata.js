...
, { event: 'disconnect', args: ['code', 'stream_name', 'reason'], reason: true }
];
...
module.exports = function ondata(data) {
  var for_user, obj, i, len, i2, len2, message, args, reason, key;
  ...
  for (i = 0, len = STREAM_MESSAGES.length; i < len; i++) {
    ...
    if (obj = data[message.event]) {
      ...
      args = [reason ? obj.reason : message.event];
      if (for_user) {
        args[1] = for_user;
      }
