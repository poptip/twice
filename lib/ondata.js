...
module.exports = function ondata(data) {
  var for_user, obj, i, len, i2, len2, message, args, key;
  ...
  obj = data.disconnect;
  // not all of the codes are documented.
  // for now we'll be listening only for the ones we know.
  // for the rest, there will be an error so we can learn what they are
  if (obj && (obj.code === 6 || obj.code === 7)) {
    args = [obj.reason].concat(obj.stream_name.split('-'));
    args.push(obj.code);
    this.emit.apply(this, args);
    return;
  }
