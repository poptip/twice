...
exports.initParamMethods = function(stream, param) {
  ...
  if (stream.params[param]) {
    var values = stream.params[param].split(',').filter(function(phrase) {
      if (phrase) {
        hash[phrase] = true;
        return true;
      } else {
...
exports.createParamMethods = function(Stream, param) {
  ...
  Stream.prototype[add] = function(values) {
    ...
    for (var i = 0, len = values.length; i < len; i++) {
      var value = values[i];
      if (this[has](value)) {
        throw Error('Already ' + param + 'ing ' + value);
      } else {
        hash[value] = true;
      }
    }
    this[count] += len;
    this.params[param] = Object.keys(hash).join(',');
  ...
  Stream.prototype[remove] = function(values) {
    ...
    for (var i = 0, len = values.length; i < len; i++) {
      var value = values[i];
      if (!this[has](value)) {
        throw Error('Not ' + param + 'ing ' + value);
      } else {
        delete hash[value];
      }
    }
