 ...
 * @return {EventEmitter}
 */
Stweam.prototype.post = function(url, body, callback) {
  if (!body) {
    body = '';
  } else {
 ...
 * @return {EventEmitter}
 */
Stweam.prototype.getTimeline = function(url, params, callback) {
 ...
 * @param {Function(!Error, Array.<Object>)} callback
 */
Stweam.prototype._getTimeline = function(ee, url, params, results, callback) {
  var orig = _.clone(params);
  ...
  var self = this;
  var fullurl = url + '?' + querystring.stringify(params);
  this.get(fullurl, function(err, tweets) {
    if (err) {
      if (callback) {
        callback(err);
      } else {
...
/**
 * Used for adding timeline functions to Stweam.
 *
 * @param {String} key
...
function addTimeline(key, resource) {
  Stweam.prototype[key] = function(params, callback) {
