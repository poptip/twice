...
var SiteStream = module.exports = function(client, follow, params) {
  ...
  self.on('beforeConnect', function() {
    ...
    if (self._users.length) {
      self._init(self._users);
    }
  ...
  self.on('control', function(uri) {
    console.log('got the control', uri);
    ...
    if (self._queuedUsers.length) {
      self.addUsers(self._queuedUsers);
    }
  });
...
SiteStream.prototype._addTwitterIDs = function(twitterIDs) {
  ...
  self.info(function(err, data) {
    ...
    data.info.users.forEach(function(user) {
      ...
      self._usersHash = user.name;
    });

    self._users = data.info.users.map(function(user) {
      return user.id;
    });
...
SiteStream.prototype.userCount = function() {
  return this._users.length + this._queuedUsers.length;
};
...
SiteStream.prototype.hasUser = function(twitterID) {
  return !!this._usersHash[twitterID] || !!this.queuedUsershash[twitterID];
};
...
SiteStream.prototype.removeUser = function(twitterID, callback) {
  ...
  self._client.post(self.remove_user_uri, { user_id: twitterID }, function(err) {
    if (err) {
    ...
    self._users.splice(self._users.indexOf(twitterID), 1);
    delete self._usersHash[twitterID];
