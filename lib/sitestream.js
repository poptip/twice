...
SiteStream.prototype._init = function(twitterIDs) {
  ...
  this.usersInStream = [];
  this.usersInStreamHash = {};
  this.usersInQueue = twitterIDs;
  this.failedToAddUsers = [];
  this.failedToAddUsersHash = {};
...
SiteStream.prototype._addUsers = function(twitterIDs) {
  ...
  self.post('add_user', params, function onAddUser(err) {
    if (err) {
      ...
      err.usersInStream = self.usersInStream.length;
      err.usersInQueue = self.usersInQueue.length;
      err.failedToAddUsers = self.failedToAddUsers.length;
      self.emit('error', err);
...
SiteStream.prototype._addTwitterIDs = function(twitterIDs) {
  ...
  self.info(function(err, data) {
    ...
    var usersHash = self.usersInStreamHash = {};
    ...
    twitterIDs.forEach(function(twitterID) {
      ...
      if (usersHash[twitterID]) {
        ...
        delete self.failedToAddUsersHash[twitterID];
      } else {
        failedToAdd.push(twitterID);
        self.failedToAddUsersHash[twitterID] = true;
      }
    });

    self.failedToAddUsers = Object.keys(self.failedToAddUsersHash);
...
SiteStream.prototype.hasUserInStream = function(twitterID) {
  return !!this.usersInStreamHash[twitterID];
};
...
SiteStream.prototype._removeUser = function(twitterID) {
  ...
  if (this.usersInStreamHash[twitterID]) {
    this.usersInStream.splice(this.usersInStream.indexOf(twitterID), 1);
    delete this.usersInStreamHash[twitterID];
